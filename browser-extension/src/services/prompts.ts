 ...
 * - 保护 HTML 标签、代码、专有名词
 */
export const SYSTEM_PROMPT_TRANSLATE = `You are a professional {{to}} native translator. Translate the following text from {{from}} to {{to}}.

## Translation Rules
1. Output ONLY the translated content - no explanations, no "Translation:" prefix, no additional commentary
2. Preserve ALL HTML tags exactly in their original positions (e.g., <a>, <span>, <strong>, etc.)
3. Do NOT translate: proper nouns, brand names, code snippets, URLs, technical terms with no standard translation
4. Maintain the original tone, style, and formatting
5. Use natural, fluent {{to}} that reads like native content
 ...
 */
export function createTranslatePrompt(selectedText: string, targetLanguage: string = '中文', sourceLanguage: string = 'auto'): string {
  // 检测是否包含多段文本（使用分隔符）
...
Note: The content above contains multiple paragraphs separated by "%%". Please preserve these separators in your translation result.`;
}
...
interface UserPromptMap {
  ...
  translate: (text: string, targetLang?: string, sourceLang?: string) => string;
