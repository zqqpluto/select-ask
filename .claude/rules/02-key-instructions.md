...
用户反馈问题时，**必须严格按以下闸门顺序执行，不可跳过任何一步，不可合并步骤**：

### 闸门 1：复现问题（第一条回复只能是复现结果，不得包含分析或修复）
...
**拦截条件**：未完成复现，不得写任何分析代码，不得提出修复方案。
**无法复现时**：必须明确告知用户"无法复现"并请求更多细节，不得自行假设问题。

### 闸门 2：分析根因（仅当闸门 1 通过后方可执行）
...
**拦截条件**：未确认根因，不得写任何修复代码。

### 闸门 3：最小修复（仅当闸门 2 通过后方可执行）
...
**拦截条件**：修复完成后，不得提交代码，必须先执行闸门 4。

### 闸门 4：再次复现验证（仅当闸门 3 通过后方可执行）
...
**循环规则**：
- 第 4 步未通过 → 回到闸门 2 重新分析 → 再修复 → 再复现，直到通过
- 第 4 步通过 → 才能执行自测、提交等后续步骤

**违反后果**：
- 跳过复现直接分析 = 代码作废，重头开始
- 未验证修复就 commit = 用户要求立即补测 + 承认错误
- 同一错误重复出现 = 流程失效，需要重新审视规则
